...
void InkHUD::MessageStore::saveToFlash()
{
    ...
    for (uint8_t i = 0; i < messages.size() && i < MAX_MESSAGES_SAVED; i++) {
        ...
        f.write(reinterpret_cast<const uint8_t *>(m.text.c_str()), min(MAX_MESSAGE_SIZE, (uint32_t) m.text.size())); // Write message text
        f.write('\0');                                                                                    // Append null term
        LOG_DEBUG("Wrote message %u, length %u, text \"%s\"", static_cast<uint32_t>(i), min(MAX_MESSAGE_SIZE, (uint32_t) m.text.size()),
                  m.text.c_str());
    }
